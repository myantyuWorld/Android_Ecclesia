...
public class ReserveCheckView extends View {
    ...
    private void onDrawBasic(Canvas canvas) {
        canvas.drawRect(0, 1080, 0, 1920, p_line);
        float room = 120;
        float room_y = 70;
        p_text.setTextAlign(Paint.Align.LEFT);
        // sx = 左上 sy　=　左下 ex　=　幅 ey　=　右下
        canvas.drawLine(0, room, 1080, room, p_out_line);
        // 各項目を交互に、色違いにするロジック
        float padding = 3;
        for (int i = 1; i < 13; i += 2) {
            canvas.drawRect(0 + padding, room * i - room + padding, 1080 - padding, room * i - padding, p_rect);
        }

        for (int i = 1; i <= 11; i++) {
            canvas.drawLine(0, room * i, 1080, room * i, p_out_line);
            canvas.drawText(name[i - 1], 100, room_y, p_text);       // 項目の描画
//            c.drawText(content[i - 1], 500, room_y, p_text);    // 予約情報の描画
            room_y += room;
        }
    }

    private void onDrawReserveInfo(Canvas canvas) {
        float y_name = 70;
        float y_purpose = 190;
        float y_start = 310;
        float y_end = 430;
