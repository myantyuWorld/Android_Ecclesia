...
CREATE TABLE IF NOT EXISTS `t_reserve` (
	...
	`pur_id`	TEXT,
	`re_applicant`	TEXT,
	FOREIGN KEY(`re_applicant`) REFERENCES `t_emp`(`emp_id`),
	FOREIGN KEY(`com_id`) REFERENCES `m_company`(`com_id`),
	PRIMARY KEY(`re_id`),
	FOREIGN KEY(`pur_id`) REFERENCES `m_purpose`(`pur_id`)
);
INSERT INTO `t_reserve` (re_id,re_overview,re_startday,re_endday,re_starttime,re_endtime,re_switch,re_fixture,re_remarks,re_priority,com_id,emp_id,room_id,pur_id,re_applicant) VALUES ('0001','新規プロジェクト報告','2018/01/17','2018/01/17','08：00','09：00',1,'プロジェクタ','なし',7,'0001','0002','0001','0001','0002'),
...
CREATE TABLE IF NOT EXISTS `t_member` (
	`re_id`	TEXT,
	`mem_id`	TEXT,
	PRIMARY KEY(`re_id`,`mem_id`)
);
INSERT INTO `t_member` (re_id,mem_id) VALUES ('0001','0001'),
 ('0001','0002'),
 ('0001','0003'),
 ('0001','0004'),
 ('0001','1000'),
...
CREATE TABLE IF NOT EXISTS `t_extension` (
	...
	`ex_starttime`	TEXT,
	`ex_endtime`	TEXT,
	FOREIGN KEY(`re_id`) REFERENCES `reserve`(`re_id`),
	PRIMARY KEY(`re_id`)
);
CREATE TABLE IF NOT EXISTS `t_emp` (
	...
	`pos_id`	TEXT,
	PRIMARY KEY(`emp_id`),
	FOREIGN KEY(`pos_id`) REFERENCES `m_position`(`pos_id`)
);
INSERT INTO `t_emp` (emp_id,emp_name,emp_tel,emp_mailaddr,dep_id,pos_id) VALUES ('0001','石山大樹','0823123456','5151002@st.hsc.ac.jp','0001','0004'),
...
CREATE TABLE IF NOT EXISTS `m_terminal` (
	`ter_id`	TEXT,
	`emp_id`	TEXT,
	PRIMARY KEY(`ter_id`),
	FOREIGN KEY(`emp_id`) REFERENCES `t_emp`(`emp_id`)
);
INSERT INTO `m_terminal` (ter_id,emp_id) VALUES ('0','0002'),
 ('1','9999'),
...
INSERT INTO `m_room` (room_id,room_name,room_capacity) VALUES ('0001','特別会議室',50),
 ('0002','会議室Ａ',20),
 ('0003','会議室Ｂ',10),
 ('0004','会議室Ｃ',5);
CREATE TABLE IF NOT EXISTS `m_purpose` (
	`pur_id`	TEXT,
	`pur_name`	TEXT,
	`pur_priority`	INTEGER,
	PRIMARY KEY(`pur_id`)
);
INSERT INTO `m_purpose` (pur_id,pur_name,pur_priority) VALUES ('0001','事業方針',7),
 ('0002','プロジェクト報告',6),
 ('0003','決算報告',5),
 ('0004','契約',4),
 ('0005','商談',3),
...
CREATE TABLE IF NOT EXISTS `m_position` (
	`pos_id`	TEXT,
	`pos_name`	TEXT,
	`pos_priority`	INTEGER,
	PRIMARY KEY(`pos_id`)
);
INSERT INTO `m_position` (pos_id,pos_name,pos_priority) VALUES ('0001','取締役会長',9),
 ('0002','代表取締役',8),
 ('0003','取締役',7),
 ('0004','執行役員',6),
 ('0005','部長',5),
 ...
 ('0008','主任',2),
 ('0009','一般社員',1);
CREATE TABLE IF NOT EXISTS `m_out` (
	`out_id`	TEXT,
	`out_name`	TEXT,
	`out_tel`	TEXT,
	`out_mailaddr`	TEXT,
	`dep_name`	TEXT,
	`pos_name`	TEXT,
	`outemp_priority`	INTEGER,
	`com_id`	TEXT,
	FOREIGN KEY(`com_id`) REFERENCES `m_company`(`com_id`),
	PRIMARY KEY(`out_id`,`outemp_priority`)
);
INSERT INTO `m_out` (out_id,out_name,out_tel,out_mailaddr,dep_name,pos_name,outemp_priority,com_id) VALUES ('1001','小俣 大輔 ','0435782511','test@test.com','開発部','係長',3,'0015'),
...
CREATE TABLE IF NOT EXISTS `m_emp` (
	...
	`pos_id`	TEXT,
	`com_id`	TEXT,
	FOREIGN KEY(`dep_id`) REFERENCES `m_depart`(`dep_id`),
	FOREIGN KEY(`com_id`) REFERENCES `m_company`(`com_id`),
	PRIMARY KEY(`emp_id`),
	FOREIGN KEY(`pos_id`) REFERENCES `m_position`(`pos_id`)
);
INSERT INTO `m_emp` (emp_id,emp_name,emp_tel,emp_mailaddr,dep_id,pos_id,com_id) VALUES ('0001','test','1234567','test@test.com','0001','0001','0000');
CREATE TABLE IF NOT EXISTS `m_depart` (
	`dep_id`	TEXT,
	`dep_name`	TEXT,
	PRIMARY KEY(`dep_id`)
);
INSERT INTO `m_depart` (dep_id,dep_name) VALUES ('0001','広報部'),
 ('0002','開発部'),
 ('0003','総務部'),
 ('0004','企画部'),
 ('0005','生活部'),
...
INSERT INTO `m_company` (com_id,com_name) VALUES ('0001','株式会社Ostraca'),
 ('0002','株式会社Ecclesia'),
 ('0003','株式会社REI'),
 ('0004','エイダエンジニアリング'),
 ('0005','アタカ工業'),
...
INSERT INTO `m_admin` (admin_id,admin_pass,emp_id) VALUES ('123456','123456','9999');
CREATE VIEW v_reserve_out_member as 
...
a.com_id ,
b.com_name 
from t_reserve x inner join t_member y on x.re_id = y.re_id inner join m_out as a on y.mem_id = a.out_id inner join m_company as b on a.com_id = b.com_id;
...
inner join t_emp z on y.mem_id = z.emp_id 

inner join m_depart as a on z.dep_id = a.dep_id 
inner join m_position as b on z.pos_id = b.pos_id 
inner join m_purpose as p on x.pur_id = p.pur_id;
CREATE VIEW v_out_emp as 
...
z.com_name
 ...
 from (t_member x inner join m_out_emp y on x.mem_id = y.outemp_id)
 inner join m_company z on y.com_id = z.com_id;
...
p.pos_name, 
p.pos_priority 
from t_emp as e inner join m_depart as d on e.dep_id = d.dep_id
inner join m_position as p on e.pos_id = p.pos_id;
COMMIT;
